(() => {
  ...
  function renderRule(rule) {
    ...
    const rebuildOperators = () => { const field = catalog.fields.find(item => item.id === get("field").value); get("operator").replaceChildren(...field.operators.map(item => option(item))); selectValue(get("operator"), rule.condition.operator); get("value").type = field.type === "number" ? "number" : "text"; if (field.type === "boolean") get("value").setAttribute("list", "boolean-values"); else get("value").removeAttribute("list"); };
    rebuildOperators(); get("value").value = String(rule.condition.value);
    ...
    get("action-type").addEventListener("change", rebuildTarget); get("operation").addEventListener("change", rebuildTarget);
  ...
  function readRule(node) {
    ...
    const value = field.type === "number" ? Number(rawValue) : field.type === "boolean" ? rawValue === "true" : rawValue;
    ...
    return { id: get("rule-id").textContent, label: get("label").value, enabled: get("enabled").checked, event: { type: field.eventType, field: field.id }, condition: { operator: get("operator").value, value }, action };
  }
  ...
  function updateStatus(status) { $("runtime-badge").textContent = status.running ? "Serverで稼働中" : "停止"; $("runtime-badge").className = `badge ${status.running ? "online" : "error"}`; $("rule-count").textContent = status.ruleCount; $("enabled-count").textContent = status.enabledRuleCount; $("last-action").textContent = status.runtime.lastResult ? `${status.runtime.lastResult.ok ? "成功" : "失敗"} / ${new Date(status.runtime.lastResult.at).toLocaleTimeString()}` : "—"; }
